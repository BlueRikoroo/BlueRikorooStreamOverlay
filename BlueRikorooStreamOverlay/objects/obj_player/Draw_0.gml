...
case Anim.idle:
	if anim_idle == spr_player_metal{
		draw_sprite_ext(anim_idle, anim_frame, x, y-14*scale, facing*scale, scale, -3*x, -1, 1)
	}else{
...
case Anim.idleRandom:
	if anim_idle == spr_player_metal{
		draw_sprite_ext(anim_idleRandom, anim_frame, x, y-14*scale, facing*scale, scale, -3*x, -1, 1)
	}else{
...
case Anim.fly:
	...
	case AnimFly.directional:
		draw_sprite_ext(anim_fly, anim_frame, x, y-13*scale, scale, scale, direction, -1, 1)
	...
	case AnimFly.gradual:
		draw_sprite_ext(anim_fly, clamp(round(vspeed*0.25/scale)+2, 0, 4), x, y, facing*scale, scale, 0, -1, 1)
		break
	case AnimFly.rotate:
		draw_sprite_ext(anim_fly, 0, x, y-14*scale, facing*scale, scale, -anim_frame*facing, -1, 1)
		break
	}
