...
if timer < 60*6{
	if obj_main.notificationEffects >= 2{
		...
		draw_text_outline(gui_xpos, gui_ypos+100, username + " raided\nwith " + string(raidAmount) + " viewers", 4, c_black, c_white)
	}else if obj_main.notificationEffects == 1{
		draw_text_setup(fnt_notification, fa_right, fa_top)
		draw_text_outline(1880, 40, username + " raided\nwith " + string(raidAmount) + " viewers", 2, c_black, c_white)
	}
}